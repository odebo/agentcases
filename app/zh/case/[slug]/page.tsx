...
export default async function CasePageZh({ params }: Props) {
  const { slug } = await params
  const c = getCaseBySlug(slug)
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="text-[11px] text-[var(--text-muted)] mb-3">
          案例研究 · <span className="text-[var(--accent-blue)]">{c.source}</span> · {new Date(c.date).toLocaleDateString('zh-CN')}
        </div>
        <h1 className="text-2xl font-bold text-[#e6edf3] leading-snug mb-1">{c.title_zh}</h1>
        <p className="text-sm text-[var(--text-dim)] mb-4">{c.title_en}</p>
        ...
        <AiComment text={c.comment_zh} lang="zh" />
